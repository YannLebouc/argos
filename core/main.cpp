...
int main()
{
    ...
    // First socket implementation will be manual and minimalistic for speed
    // and educational purpose Second version will use the modern posix API
    // with getaddrinfo() etc...
    ...
    while (true) {
        int new_socket_fd =
            accept(socket_fd, (struct sockaddr *)&client_addr_info, &client_addr_info_size);
        if (new_socket_fd == -1) {
        ...
        while (true) {
            ...
            while ((newline_pos = stream_buffer.find('\n')) != std::string::npos) {
                ...
                std::cout << "Complete message received and treated : " << message << std::endl;
